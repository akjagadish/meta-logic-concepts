...
class Trainer:
    ...
    def evaluate(self, eval_datasplit=None, name="Validation", save=False):
        ...
        for batch_index, batch in enumerate(eval_datasplit):
            loss, acc, correct_array = self.prediction_step(self.model, batch)
            loss = loss.item()
            ...
            first_25 = correct_array[: int(0.25 * len(correct_array))]
            if len(first_25) == 0:
                first_25 = [correct_array[0]]
            last_25 = correct_array[-int(0.25 * len(correct_array)) :]
            if len(last_25) == 0:
                last_25 = [correct_array[-1]]

            first_array.append(np.sum(first_25)/len(first_25))
            last_array.append(np.sum(last_25)/len(last_25))
        ...
        logging.info("accuracy on first 25%% examples " + str(mean(first_array)))
        logging.info("accuracy on last 25%% examples " + str(mean(last_array)))
...
class MetaTrainer(Trainer):
    ...
    def prediction_step(self, model, inputs):
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            # print('test in', test['input_ids'].shape)
            # print('test labels', test['labels'].shape)
            # Evaluate on the test set for this episode
            outputs = fmodel(test)
            outer_loss = outputs["loss"].detach()
            outer_acc = outputs["accuracy"]
            correct_array = outputs["correct_array"].detach().numpy()
            # print('test data', test['input_ids'])
            # print('corect array in pred step', correct_array)

            return outer_loss, outer_acc, correct_array

class MetaTrainerOneExample(MetaTrainer):

    def __init__(self, ignore_first=False, **kwargs):
        super(MetaTrainerOneExample, self).__init__(**kwargs)
    ...
    def training_step(self, model, inputs):
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            with higher.innerloop_ctx(model, self.inner_opt, copy_initial_weights=False) as (fmodel, diffopt):
                ...
                # Train on the training set for this episode
                for epoch in range(self.epochs_per_episode):
                    for (i, mini_batch) in enumerate(inputs["train_batches"]):
                        if not(self.ignore_first and i == 0):
                            if self.multi_step_loss:
                                this_loss = 0
    ...
    def prediction_step(self, model, inputs):
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            for epoch in range(self.epochs_per_episode):
                for (i, mini_batch) in enumerate(inputs["train_batches"]):
                    test = mini_batch
                    outputs = fmodel(test, reduction='sum')
                    if not(self.ignore_first and i == 0):
                        outer_loss += outputs["loss"].detach()
                        num_examples = torch.sum(mini_batch['labels'] != 2).item()
                        outer_acc += outputs["accuracy"] * num_examples
                        outer_acc_denom += num_examples
                        correct_array = outputs["correct_array"]
                        correct_arrays += correct_array
            ...
            return outer_loss / outer_acc_denom, outer_acc / outer_acc_denom, correct_arrays
...
def simple_wudsy_model(model, inputs, lr=0.1, epochs=1, vary_train_batch_size=False, ignore_first=False):
        ...
        with torch.backends.cudnn.flags(enabled=False):
            ...
            fmodel = copy.deepcopy(model)
            diffopt = torch.optim.SGD(fmodel.parameters(), lr=lr)
            ...
            for epoch in range(epochs):
                for (i, mini_batch) in enumerate(inputs["train_batches"]):
                    ...
                    inner_loss = fmodel(mini_batch)["loss"]
                    correct_array = fmodel(mini_batch)["correct_array"]
                    if not(ignore_first and i == 0):
                        correct_arrays += correct_array
                    inner_loss.backward()
                    diffopt.step()
                    fmodel.zero_grad()

            return {'loss': outer_loss / outer_acc_denom, 'accuracy': outer_acc / outer_acc_denom, 
                    'correct_array': correct_arrays}
